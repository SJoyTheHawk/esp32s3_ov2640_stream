...
.message{min-height:1.3em;margin:0}.error{color:#fca5a5}.ok{color:#86efac}.notice{color:#fcd34d}.muted{color:#9da5ad;font-size:.85rem;margin:7px 0 0}.hidden{display:none}.modal{display:none;position:fixed;inset:0;z-index:10;background:rgba(0,0,0,.72);padding:18px;align-items:center;justify-content:center}.modal.show{display:flex}.modal-content{width:min(560px,100%);max-height:90vh;overflow:auto;background:#181c20;border:1px solid #4a5158;border-radius:8px;padding:22px;box-shadow:0 16px 48px rgba(0,0,0,.45)}.modal-header{display:flex;justify-content:space-between;align-items:center;gap:12px}.modal-header h2{margin:0}.close-btn{margin:0;padding:2px 8px;background:transparent;color:#b2bac2;font-size:1.7rem;line-height:1}.close-btn:hover{background:#343a40;color:#fff}.setting-links{display:grid;grid-template-columns:1fr 1fr;gap:14px}.setting-link{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:14px 0}.setting-link p{margin:4px 0 0}.danger{font-size:1rem;padding:12px 20px}@media(max-width:600px){main{padding:18px}.grid,.status,.setting-links{grid-template-columns:1fr}.full{grid-column:auto}}
...
<section id="admin-network" class="admin-only"><h2>Network</h2><div class="setting-link"><div><strong>WiFi configuration</strong><p class="muted" id="wifi-summary">Manage SSID and network address</p></div><button type="button" class="quiet" id="wifi-open">Configure</button></div></section>
<section><h2>Security</h2><div class="setting-link"><div><strong>Account password</strong><p class="muted">Update the password for this account</p></div><button type="button" class="quiet" id="password-open">Change</button></div></section>
